...
//class Wechat extends \yii\base\Component {
    ...
    //http://backend.hoyatech.net/index.php?r=wx/wap/oauth2cb
    ...
    public function getGh() {
        ...
        $db = \Yii::$app->db;
        $tableName = \backend\modules\wx\models\MGh::tableName();
    ...
    public function getAccessToken($forceRefresh = false) {
        ...
        if (is_array($tmpaccesstoken) && !empty($tmpaccesstoken['expire']) && $tmpaccesstoken['expire'] > time()) {
            return $this->_accessToken;
        } else {
            $arr = U::curl("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=" . $gh['appid'] . '&secret=' . $gh['appsecret']);
            $token = @json_decode($arr, true);
            ...
            $mgh->access_token = serialize($record);
            $mgh->save();
            $this->_accessToken = $record['access_token'];
            return $this->_accessToken;
        }
        /* if (!$forceRefresh) {
    ...
    public function getPostStr() {
        if ($this->localTest) {
            ...
            return $this->getDemoRequestXml(Wechat::MSGTYPE_EVENT, Wechat::EVENT_CLICK, 'FuncMyActivity');
            //return $this->getDemoRequestXml(Wechat::MSGTYPE_EVENT, Wechat::EVENT_LOCATION_SELECT, 'FuncMySchoolbranch');				
    ...
    public function sendTemplateMessage($data, $touser, $templateId, $url, $token, $topcolor = '#FF0000') {
        ...
        return U::curl($queryUrl, $template);
    }
    ...
    public function getDemoRequestXml($MsgType, $Event = Wechat::EVENT_SUBSCRIBE, $EventKey = 'FuncQueryAccount') {
        $openid = MGh::GH_HOYA_OPENID_HBHE;
        $gh_id = $this->getGhId();
        switch ($MsgType) {
            case Wechat::MSGTYPE_TEXT:
                $xml = <<<EOD
...
EOD;
    ...
    public function WxGetOauth2Url($scope = 'snsapi_userinfo', $state = '') {
        ...
        //$redirect_uri = urlencode("http://wosotech.com/wx/web/index.php?r=wap/oauth2cb");    
        //$redirect_uri = urlencode("http://backend.hoyatech.net/index.php?r=wx/wap/oauth2cb");        
...
//            return $paraString . "&sign=" . $this->sign($unSignParaString,self::trimString(PARTNERKEY));
            \backend\modules\wx\models\U::W([$partnerkey, $this->parameters]);
            return $paraString . "&sign=" . $this->sign($unSignParaString,self::trimString($partnerkey));
    }
    ...
    protected function get_biz_sign($bizObj){
         foreach ($bizObj as $k => $v){
             ...
             //var_dump($bizString);
            \backend\modules\wx\models\U::W($bizParameters);
             return sha1($bizString);
    }
