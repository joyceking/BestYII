  ...
  <div data-role="content">
    ...
    <ul id="dist_list" data-role="listview"  class="ui-nodisc-icon ui-alt-icon" data-filter="true" data-filter-placeholder="Search ..." data-inset="true">
        ...
        <?php
            ...
            $schoolregions = ["江岸区","江汉区","硚口区","汉阳区","武昌区","洪山区","青山区","蔡甸区","汉南区","江夏区","黄陂区","新洲区","东西湖区"];

            foreach($schoolregions as $schoolregion)
            {
                $n = 0;
                foreach($schoolbranchs as $schoolbranch)
                {
                    if($schoolbranch->region == $schoolregion)
                    {
                        $n = $n + 1;
                    }
                }
        ...
        ?>
            <li>
                <a data-ajax='false' href='http://backend.hoyatech.net/index.php?r=wx/yss/schoolbranch&gh_id=<?php echo $gh_id;?>&openid=<?php echo $openid;?>&region=<?php echo $schoolregion;?>'><h2><?= $schoolregion ?></h2><span class='ui-li-count'><?= $n ?></span></a>
            </li>
