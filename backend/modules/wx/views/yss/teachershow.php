  ...
  <div data-role="content">
    ...
    <ul data-role="listview" class="ui-nodisc-icon ui-alt-icon" data-filter="true" data-filter-placeholder="Search ..." data-inset="true">
         ...
         <?php
            ...
            ?>
               <li>
                <a data-ajax='false' href="http://backend.hoyatech.net/index.php?r=wx/yss/teachershowdetail&gh_id=<?php echo $gh_id;?>&openid=<?php echo $openid;?>&teacher_id=<?php echo $teacher->teacher_id;?>">
               
                  <img src="<?= $teacher->getHeadUrl(); ?>" alt="">
...
<script>
  ...
  var shareData = {
    ...
    link: 'http://backend.hoyatech.net/index.php?r=wx/yss/teachershow&gh_id=<?php echo $gh_id;?>&openid=nobody',
...
/*
    <ul data-role="listview" class="ui-nodisc-icon ui-alt-icon" data-filter="true" data-filter-placeholder="Search ..." data-inset="true">
         ...
         <?php
            ...
            foreach($teachers as $teacher)
            {           
              $photos = MPhotoOwner::getPhotosByOwner(MPhotoOwner::PHOTO_OWNER_TEACHER, $teacher->teacher_id, 10); 
              //$models = MPhotoOwner::getPhotosByOwner(MPhotoOwner::PHOTO_OWNER_TEACHER, $teacher->teacher_id, 1); 
              $photos_tag = [];

              foreach($photos as $photo) {

                  if (mb_stripos($photo->tags, '头像') !== false)
                    $photos_tag[] = $photo;
              }

              $photo = empty($photos_tag) ? MPhoto::newPhoto('', '', '/uploads/anonymous.jpg') : $photos_tag[0];                        
            ?>
               <li>
                <a data-ajax='false' href="http://backend.hoyatech.net/index.php?r=wx/yss/teachershowdetail&gh_id=<?php echo $gh_id;?>&openid=<?php echo $openid;?>&teacher_id=<?php echo $teacher->teacher_id;?>">
               
                  <img src="<?php echo Url::to($photo->getPicUrl(160,160)) ?>" alt="">
